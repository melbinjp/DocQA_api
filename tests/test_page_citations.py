...
PyMuPDF only when MarkItDown returned *nothing*, so a PDF read badly rather than
not at all never reached the fallback. The deployed Space returned arXiv
2005.11401 with the spaces removed; the same PDF and the same pinned
`markitdown==0.1.6` extract cleanly locally, because `markitdown[pdf]` pulls an
unpinned `pdfminer.six` that each image resolves for itself. Text quality
therefore varied by deployment. Silent mangling is worse than failure: a mangled
chunk still embeds, still retrieves, and still gets quoted back to a user.
"""
